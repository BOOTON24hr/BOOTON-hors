...
const config = {
    A187: '📍WWILLIAMBOOT²⁴hrs😎📍',
    instagram: 'OFF',
    nomer: 'wa.me/14991357606',
    aktif: '24 JAM',
...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code is ready, se escreva Aris187 ID`)
})
...
client.on('message-new', async (m) => {
   ...
   const prefix = tipo da mensagem === 'imageMessage' ? imageMessage.caption.split(' ')[0].split(re)[1] : text.split(' ')[0].split(re)[1] // prefixo múltiplo

   console.log(`[ ${time} ] => Nomor :[ ${id.split("@s.whatsapp.net")[0]} ] => ${text}`);

   switch (prefix) {
       case 'help':
           client.enviar massagem(id, help.help(id, A187, tanggal, waktu, whatsapp, youtube, instagram, aktif, nomer, ontime), tipo de mensagem.text)
           break
       case 'menu1':
           client.sendMessage(id, menu1.menu1(id, A187, tanggal, waktu, whatsapp, youtube, instagram, aktif, nomer, ontime), tipo de mensagem.text)
           break
       case 'menu2':
           client.sendMessage(id, menu2.menu2(id, A187, tanggal, waktu, whatsapp, youtube, instagram, aktif, nomer, ontime), tipo de mensagem.text)
           break
       case 'menu3':
           client.sendMessage(id, menu3.menu3(id, A187, tanggal, waktu, whatsapp, youtube, instagram, aktif, nomer, ontime), tipo de mensagem.text)
           break
      case 'donate':
           client.sendMessage(id, donate.donate(id, A187, tanggal, waktu, whatsapp, youtube, instagram, aktif, nomer, ontime), tipo de mensagem)
           break          
      case 'info':
       ...
       case 'nulis':
           nulis(value)
               .then(data => {
                   client.sendMessage(id, '[PERA AI TA QUASE PRONTO👍', tipo de mensagem)
                   client.sendMessage(id, data, tipo de mensagem.image)
               })
               .catch(err => {
                   console.log(err)
               })
       ...
       case 'ytmp3':
           ytdl('mp3', value)
               .then(data => {
                   ...
                   client.sendMessage(id, '[ESPERE AI TA QUASE PRONTO👍 POUCO...', MessageType.text)
                   client.sendMessage(id, hasil, 
tipo de mensagem)
               })
               .catch(err => {
                   console.log(err)
...
tipo de mensagrm.text);
       ...
       case 'pantun':
           pantun()
               .then(data => {
                   client.sendMessage(id, data, 
tipo de mesagem .text)
               })
               .catch(err => {
                   console.log(err)
               })
       ...
       case 'quotes':
           quotes()
               .then(data => {
                   ...
                   client.sendMessage(id, hasil, tipo de mensagem.text)
               })
               .catch(err => {
                   console.log(err)
               })
       ...
       case 'pict':
           switch (value) {
               ...
               case 'cowok':
                   cowoPict()
                       .then(buffer => {
                           client.sendMessage(id, '[ESPERE UM POUCO...', tipo de mensagem.text)
                           client.sendMessage(id, buffer, MessageType.image)
                       })
                       .catch(err => {
                           console.log(err)
